...
int bh_sync__hook_exec(const bh_daemon *daemon, const bh_error *error,
                       const char *event);
void bh_sync__counter_reset(const bh_daemon *daemon, ...);

int bh_run_sync(bh_daemon *daemon) {
  int ret_code = 0;
...
#define otherwise(events...)                                                   \
  if (false == event_raised) {                                                 \
    if (0 > ret_code)                                                          \
      bh_sync__handle_error(daemon, BH_EVENT_FATALERRORS);                     \
    else                                                                       \
      bh_print_ok();                                                           \
  }                                                                            \
  return_err(ret_code);                                                        \
  ...
  ret_code = bh_git_commit_changes(&new_commit_after_commit, daemon->manager,
                                   new_commit_after_merge, daemon->signature);
...
cleanup:
  bh_print_end();
  if (NULL != new_commit_after_merge)
    git_commit_free(new_commit_after_merge);
  if (NULL != new_commit_after_commit)
    git_commit_free(new_commit_after_commit);
  if (NULL != current_ref)

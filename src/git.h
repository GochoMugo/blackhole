 ...
 *
 * @param manager Repository manager
 * @return 0 on success, BH_GITERR_NO_UPDATES if no updates were found or error code
 */
 ...
 * @param manager Repository manager
 * return 0 on success, BH_GITERR_MERGE_CONFLICTS if conflicts occur or any other error code
 */
int bh_git_merge_origin(git_commit **out, bh_git_repository_manager *manager);


/**
 * Commit local changes in the repository. This adds and updates files, respecting
 * the ignore rules.
 *
 * @param new_commit Newly-created commit
 * @param manager Repository manager
 * @param commit Commit from `bh_git_merge_origin()`. If NULL, use commit pointed by HEAD
 * @return 0 on success, BH_GITERR_NO_CHANGES if no local changes are available or error code
 */
 ...
 */
int
bh_git_push(bh_git_repository_manager *manager,
 ...
 */
int
bh_git_checkout_branch(bh_git_repository_manager *manager,
 ...
 */
int
bh_git_is_dirty(int *is_dirty, bh_git_repository_manager *manager);
 ...
 *
 * @param manager Repository manager
 * @param reference_name Name of the reference e.g. HEAD, refs/heads/master, etc.
 * @param commit_id OID of the commit
 * @param reflog messaged to be used in reflog
 * @return 0 or error code
 */
int
bh_git__update_ref(bh_git_repository_manager *manager,
        const char *ref_name,
        git_oid *commit_id,
        const char *reflog);

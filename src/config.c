...
bh_config_new(bh_config **config, const char* rootdir) {
    ...
    return_err_ext(asprintf((char**) &(c->path), "%s", rootdir), BH_DAEMONERR_PATHRESLV);

    return_err_ext(contra_path_join(&bh_path, c->path, BH_PATH_ROOTDIR), BH_DAEMONERR_PATHRESLV);

    return_err_ext(contra_path_join(&config_path, bh_path, BH_PATH_CONFIG), BH_DAEMONERR_PATHRESLV);
    ...
    return_err_ext(
        contra_path_join(&(c->runstate_path), bh_path, iniparser_getstring(c->dict, "runstate:path", "")), 
...
bh_config_free(bh_config **config) {
    ...
    if (NULL != c->path) free((char*) c->path);
    if (NULL != c->runstate_path) free((char*) c->runstate_path);
    free(c);
    *config = NULL;
}

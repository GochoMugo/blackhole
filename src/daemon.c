...
int bh_daemon_new(bh_daemon **daemon, const char *path) {
  ...
  return_err_ext(contra_path_join(&(d->paths.counters),
                                  d->config->runstate_path, "counters"),
                 BH_DAEMONERR_PATHRESLV);

  /* Resolve internal path to 'counters' */
  return_err_ext(
      contra_path_join(&(d->paths.hooks), d->config->runstate_path, "hooks"),
      BH_DAEMONERR_PATHRESLV);
  ...
  return_err_ext(
      git_signature_now(&(d->signature), d->config->name, d->config->email),
      BH_DAEMONERR_SIGNATURE);
...
void bh_daemon_free(bh_daemon **daemon) {
  ...
  if (NULL == d)
    return;
  if (NULL != d->config)
    bh_config_free(&(d->config));
  if (NULL != d->paths.counters)
    free(d->paths.counters);
  if (NULL != d->paths.hooks)
    free(d->paths.hooks);
  if (NULL != d->manager)
    bh_git_repository_manager_free(&(d->manager));
  if (NULL != d->signature)
    git_signature_free(d->signature);
  free(d);
  *daemon = NULL;
}

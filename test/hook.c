...
int tests_bh_hook_teardown_each(void **state) { return 0; }
...
void tests_bh_hook_exec_missing(void **state) {
  ...
  assert_int_equal(bh_hook_exec(daemon_hooks, "404.sh", "missing"),
                   BH_HOOKERR_NO_HOOK);
}
...
void tests_bh_hook_exec_fail(void **state) {
  ...
  assert_int_equal(bh_hook_exec(daemon_hooks, "fail.sh", "fails"),
                   BH_HOOKERR_FAILED);
}
